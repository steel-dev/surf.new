...
const processLinks = (text: string | ReactNode | ReactNode[], keyOffset: number): ReactNode | ReactNode[] => {
  if (typeof text !== 'string') {
    if (Array.isArray(text)) {
      return text.map((item, index) => 
        typeof item === 'string' 
          ? processLinks(item, keyOffset + index * 100) 
          : item
      );
    }
    return text;
...
const processBold = (text: string | ReactNode | ReactNode[], keyOffset: number): ReactNode | ReactNode[] => {
  if (typeof text !== 'string') {
    if (Array.isArray(text)) {
      return text.map((item, index) => 
        typeof item === 'string' 
          ? processBold(item, keyOffset + index * 100) 
          : item
      );
    }
    return text;
  ...
  while ((match = boldRegex.exec(text)) !== null) {
    ...
    // Add the bold text
    result.push(
      <strong key={`bold-${keyOffset}-${matchIndex}`}>
...
const processItalic = (text: string | ReactNode | ReactNode[], keyOffset: number): ReactNode | ReactNode[] => {
  if (typeof text !== 'string') {
    if (Array.isArray(text)) {
      return text.map((item, index) => 
        typeof item === 'string' 
          ? processItalic(item, keyOffset + index * 100) 
          : item
      );
    }
    return text;
  ...
  while ((match = italicRegex.exec(text)) !== null) {
    ...
    const processedItalicContent = parseInlineMarkdown(italicContent, keyOffset + 4000 + matchIndex);
    
    // Add the italic text
    result.push(
      <em key={`italic-${keyOffset}-${matchIndex}`}>
...
const processStrikethrough = (text: string | ReactNode | ReactNode[], keyOffset: number): ReactNode | ReactNode[] => {
  if (typeof text !== 'string') {
    if (Array.isArray(text)) {
      return text.map((item, index) => 
        typeof item === 'string' 
          ? processStrikethrough(item, keyOffset + index * 100) 
          : item
      );
    }
    return text;
  ...
  while ((match = strikeRegex.exec(text)) !== null) {
    ...
    const processedStrikeContent = parseInlineMarkdown(strikeContent, keyOffset + 5000 + matchIndex);
...
const processInlineCode = (text: string | ReactNode | ReactNode[], keyOffset: number): ReactNode | ReactNode[] => {
  if (typeof text !== 'string') {
    if (Array.isArray(text)) {
      return text.map((item, index) => 
        typeof item === 'string' 
          ? processInlineCode(item, keyOffset + index * 100) 
          : item
      );
    }
    return text;
  ...
  while ((match = codeRegex.exec(text)) !== null) {
    ...
    result.push(
      <code 
        ...
        className="rounded bg-[--gray-2] px-1.5 py-0.5 font-mono text-sm text-[--gray-12] border border-[--gray-3]"
      >
        {codeContent}
      </code>
    );
...
const processSpoiler = (text: string | ReactNode | ReactNode[], keyOffset: number): ReactNode | ReactNode[] => {
  if (typeof text !== 'string') {
    if (Array.isArray(text)) {
      return text.map((item, index) => 
        typeof item === 'string' 
          ? processSpoiler(item, keyOffset + index * 100) 
          : item
      );
    }
    return text;
  ...
  while ((match = spoilerRegex.exec(text)) !== null) {
    ...
    const processedSpoilerContent = parseInlineMarkdown(spoilerContent, keyOffset + 6000 + matchIndex);
